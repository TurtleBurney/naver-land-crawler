...
def upgrade():
    ...
    sa.ForeignKeyConstraint(['fk_region_code'], ['region.region_code'], ondelete='CASCADE'),
    ...
    op.create_table('contract_price',
    sa.Column('contract_price_id', sa.Integer(), autoincrement=True, nullable=False),
    sa.Column('contract_type', sa.Enum('Deal', 'Jeonse', 'Wolse', name='contract_type_enum'), nullable=False),
    ...
    sa.ForeignKeyConstraint(['fk_house_id'], ['household.household_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('contract_price_id')
    )
    
    
    op.add_column('household', sa.Column('building_dong', sa.String(length=10), nullable=False))
    ...
    op.add_column('household', sa.Column('household_url', sa.String(length=200), nullable=False))
    op.add_column('household', sa.Column('fk_building_id', sa.String(length=50), nullable=True))
    op.create_foreign_key(None, 'household', 'building', ['fk_building_id'], ['building_id'], ondelete='CASCADE')
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('household', sa.Column('building_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.add_column('household', sa.Column('dong', sa.INTEGER(), autoincrement=False, nullable=False))
    op.add_column('household', sa.Column('link', sa.VARCHAR(length=200), autoincrement=False, nullable=False))
    op.add_column('household', sa.Column('area', postgresql.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=False))
    ...
    sa.Column('building_id', sa.INTEGER(), server_default=sa.text("nextval('building_basic_building_id_seq'::regclass)"), autoincrement=True, nullable=False),
    sa.Column('building_name', sa.VARCHAR(length=50), autoincrement=False, nullable=False),
    sa.Column('building_type', postgresql.ENUM('A1', 'B1', 'B2', name='building_code_enum_1'), autoincrement=False, nullable=False),
    sa.Column('region_code', sa.VARCHAR(length=50), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['region_code'], ['region.region_code'], name='building_basic_region_code_fkey', ondelete='CASCADE'),
    ...
    sa.Column('number_address', sa.VARCHAR(length=100), autoincrement=False, nullable=False),
    sa.Column('road_address', sa.VARCHAR(length=100), autoincrement=False, nullable=False),
    ...
    sa.Column('sale_type', postgresql.ENUM('Deal', 'Jeonse', 'Wolse', name='sale_type_enum'), autoincrement=False, nullable=False),
    ...
    sa.ForeignKeyConstraint(['house_id'], ['household.household_id'], name='price_house_id_fkey', ondelete='CASCADE'),
